...
def offline_ema(args):
    ...
    transformer_config = MaskGitTransformer.load_config(Path(dirs[0]) / "unwrapped_model")
    ...
    model = model_cls.from_pretrained(Path(dirs[0]) / "unwrapped_model").to(device)
    ema_model = EMAModel(parameters=model.parameters(), ema_decay=ema_decay, update_every=checkpoint_interval).to(
        device
    )

    end_step = int(dirs[-1].split("-")[1])
